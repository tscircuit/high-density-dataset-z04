...
export const calculateNodeProbabilityOfFailure = (
  ...
  const estNumVias =
    numSameLayerCrossings * 0.81 +
    numEntryExitLayerChanges * 0.36 +
    numTransitionCrossings * 0.14
  const {
    nodeArea,
    capacityRatio,
    viaAreaRatio,
...
const main = async () => {
  ...
  for (const capacityScale of scaleValues) {
    for (const viaAreaScale of scaleValues) {
      for (const crossingDensityScale of scaleValues) {
        for (const areaAdjustmentScale of scaleValues) {
          for (const areaCapacityScale of scaleValues) {
            for (const areaViaAreaScale of scaleValues) {
              const params = withScale(baselineModelParams, {
                capacityWeight: capacityScale,
                viaAreaRatioWeight: viaAreaScale,
                crossingDensityWeight: crossingDensityScale,
                areaAdjustmentWeight: areaAdjustmentScale,
                areaCapacityFactor: areaCapacityScale,
                areaViaAreaFactor: areaViaAreaScale,
              })
